...
void lnaa_sha256_transform(LNAA_SHA256_CTX *ctx, const BYTE data[])
{
	...
	for (i = 0; i < 64; ++i) {
		t1 = h + EP1(e) + CH(e,f,g) + lnaa_k[i] + m[i];
		t2 = EP0(a) + MAJ(a,b,c);
		h = g;
		g = f;
		f = e;
...
void lnaa_sha256_update(LNAA_SHA256_CTX *ctx, const BYTE data[], size_t len)
{
	...
	for (i = 0; i < len; ++i) {
		...
		ctx->datalen++;
		if (ctx->datalen == 64) {
			lnaa_sha256_transform(ctx, ctx->data);
			ctx->bitlen += 512;
			ctx->datalen = 0;
		}
	}
